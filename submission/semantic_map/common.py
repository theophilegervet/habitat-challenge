...
def init_map_and_pose_for_env(e: int,
                              local_map: Tensor,
                              global_map: Tensor,
                              local_pose: Tensor,
                              global_pose: Tensor,
    ...
    global_pose[e].fill_(0.)
    global_pose[e, :2] = p.global_map_size_cm / 100. / 2.

    # Initialize starting agent locations
    x, y = (global_pose[e, :2] * 100 / p.resolution).int()
    global_map[e].fill_(0.)
    global_map[e, 2:4, y - 1:y + 2, x - 1:x + 2] = 1.

    recenter_local_map_and_pose_for_env(
        e, local_map, global_map, local_pose, global_pose,
        lmb, origins, map_size_parameters
    )


def recenter_local_map_and_pose_for_env(e: int,
                                        local_map: Tensor,
                                        global_map: Tensor,
                                        local_pose: Tensor,
                                        global_pose: Tensor,
    ...
    origins[e] = torch.tensor([
        lmb[e][2] * p.resolution / 100.,
        lmb[e][0] * p.resolution / 100.,
        0.,
    ])
    local_map[e] = global_map[
        e, :,
        lmb[e, 0]:lmb[e, 1],
        lmb[e, 2]:lmb[e, 3]
    ]
...
def get_local_map_boundaries(global_loc: torch.IntTensor,
                             map_size_parameters: MapSizeParameters
                             ) -> torch.IntTensor:
    ...
    if p.global_downscaling > 1:
        y1, x1 = y - p.local_h // 2, x - p.local_w // 2
        y2, x2 = y1 + p.local_h, x1 + p.local_w

        if y1 < 0:
            y1 = torch.tensor(0, device=device, dtype=dtype)
            y2 = torch.tensor(p.local_h, device=device, dtype=dtype)
        if y2 > p.global_h:
            y1 = torch.tensor(p.global_h - p.local_h, device=device, dtype=dtype)
            y2 = torch.tensor(p.global_h, device=device, dtype=dtype)

        if x1 < 0:
            x1 = torch.tensor(0, device=device, dtype=dtype)
            x2 = torch.tensor(p.local_w, device=device, dtype=dtype)
        if x2 > p.global_w:
            x1 = torch.tensor(p.global_w - p.local_w, device=device, dtype=dtype)
            x2 = torch.tensor(p.global_w, device=device, dtype=dtype)
